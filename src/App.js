...
export default function App() {
    ...
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body:JSON.stringify({
          ...
          system:"You are the voice of The GWB Project. Create a single daily community challenge prompt for women focused on intentional living and growth. Make it thoughtful, specific, and inspiring. Return ONLY the question itself — under 20 words, no quotes.",
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body:JSON.stringify({
          ...
          system:`You are the voice of The GWB Project - Grow With Boldness. Speak like The Grown Woman Blueprint: refined, warm, direct. No fluff. No emojis. 2-3 sentences max. The user's aesthetic is ${aesObj?.name} - "${aesObj?.tagline}". Address them by display name. Every nudge must feel completely fresh. Seed: ${seed}.`,
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:SERIF,color:T.text,paddingBottom:"90px"}}>
      ...
      <div style={{maxWidth:"480px",margin:"0 auto",padding:"0 18px",position:"relative",zIndex:1}}>
        ...
        {view==="today"&&goals.length>0&&<div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:"14px",padding:"18px",marginBottom:"12px",display:"flex",alignItems:"center",gap:"18px"}}>
          ...
          <div>
            ...
            <p style={{fontSize:"12px",color:T.dim,fontStyle:"italic",lineHeight:1.4}}>{pct===100?"Fully aligned. You showed up.":pct>=50?"You're in motion. Keep going.":"Your day is waiting for you."}</p>
          </div>
        </div>}
        ...
        {view==="wall"&&<div>
          ...
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:"14px",padding:"14px 16px",marginBottom:"14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <p style={{fontSize:"10px",letterSpacing:"4px",color:T.muted}}>THE GWB PROJECT</p>
          ...
          <div style={{background:T.card,border:`2px solid ${gold}`,borderRadius:"14px",padding:"18px",marginBottom:"14px"}}>
            ...
            {challengeLoading?(
              <p style={{fontSize:"14px",color:T.dim,fontStyle:"italic",animation:"shimmer 1.5s infinite"}}>Generating today's challenge...</p>
            ):dailyChallenge?(
              <div>
                ...
                {!challengeSubmitted?(
                  <div style={{display:"flex",gap:"8px"}}>
                    ...
                    <button onClick={submitChallengeResponse} style={{padding:"11px 16px",background:gold,border:"none",color:T.bg,fontSize:"14px",borderRadius:"0 8px 8px 0",fontWeight:"600"}}>→</button>
                  </div>
                ):(
          ...
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:"14px",padding:"16px",marginBottom:"14px"}}>
            <div style={{display:"flex",gap:"12px",alignItems:"center"}}>
              <Avatar icon={aesObj?.icon||"◈"} color={gold} size={40}/>
              {!composerOpen?(
            ...
            {composerOpen&&<div className="fi" style={{marginTop:"14px"}}>
              ...
              {postImagePreview&&<div style={{position:"relative",marginBottom:"10px"}}>
                ...
                <button onClick={()=>{setPostImage(null);setPostImagePreview(null);}} style={{position:"absolute",top:"8px",right:"8px",background:"rgba(26,22,18,0.8)",border:"none",color:"#f0e8d8",width:"28px",height:"28px",borderRadius:"50%",fontSize:"16px"}}>×</button>
              </div>}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:"8px"}}>
                  ...
                  <button onClick={submitPost} disabled={uploadingImage||(!postContent.trim()&&!postImage)}
                    style={{padding:"9px 20px",background:gold,border:"none",borderRadius:"8px",fontSize:"12px",color:T.bg,fontWeight:"600",opacity:(!postContent.trim()&&!postImage)?0.5:1}}>
                  ...
                  <div style={{borderTop:`1px solid ${T.border}`}}>
                    ...
                    {commentsOpen&&<div style={{padding:"4px 16px 16px"}}>
                      ...
                      <div style={{display:"flex",gap:"10px",alignItems:"center",marginTop:"6px"}}>
                        <Avatar icon={aesObj?.icon||"◈"} color={gold} size={32}/>
                        <div style={{flex:1,display:"flex"}}>
                          ...
                          <button onClick={()=>submitReply(post.id)} style={{padding:"10px 14px",background:gold,border:"none",color:T.bg,fontSize:"14px",borderRadius:"0 24px 24px 0"}}>→</button>
                        </div>
                      </div>
                    </div>}
                  </div>
        ...
        {!dataLoading&&view==="cycle"&&<div>
          {!cycleSaved?(
            <div style={{background:T.card,border:"1px solid "+T.border,borderRadius:"14px",padding:"24px",marginBottom:"14px"}}>
              ...
              <button onClick={saveCycleDate} style={{width:"100%",padding:"14px",background:"#c9a84c",border:"none",color:T.bg,fontSize:"11px",letterSpacing:"4px",fontWeight:"600",borderRadius:"8px"}}>
                REVEAL MY PHASE
              </button>
            </div>
          ):(()=>{
            ...
            return (
              <div>
                ...
                <div style={{background:T.card,border:"1px solid "+T.border,borderRadius:"14px",padding:"20px",marginBottom:"14px"}}>
                  {[["ENERGY",phase.energy],["MOVEMENT",phase.workout],["NOURISHMENT",phase.foods],["GWB SAYS",`"${phase.affirmation}"`]].map(([label,val],i)=>(
        ...
        {!dataLoading&&view==="vision"&&<div>
          ...
          <div style={{background:T.card,border:"1px solid "+T.border,borderRadius:"14px",padding:"16px",marginBottom:"16px"}}>
            {visionImagePreview?(
              <div style={{position:"relative",marginBottom:"12px"}}>
                ...
                <button onClick={()=>{setVisionImage(null);setVisionImagePreview(null);}}
                  style={{position:"absolute",top:"8px",right:"8px",background:"rgba(26,22,18,0.8)",border:"none",color:"#f0e8d8",width:"28px",height:"28px",borderRadius:"50%",fontSize:"16px"}}>×</button>
              </div>
            ):(
            ...
            <button onClick={submitVisionPost} disabled={!visionImage||uploadingVision}
              style={{width:"100%",padding:"13px",background:visionImage?(aesObj?aesObj.color:T.gold):T.accent,border:"none",color:T.bg,fontSize:"11px",letterSpacing:"4px",fontWeight:"600",borderRadius:"8px",opacity:!visionImage?0.5:1}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
              {visionPosts.map(post=>(
                <div key={post.id} style={{position:"relative",borderRadius:"10px",overflow:"hidden"}}>
                  ...
                  {post.caption&&<div style={{position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(transparent, rgba(26,22,18,0.9))",padding:"20px 10px 8px"}}>
                    <p style={{fontSize:"12px",color:"#f0e8d8",fontStyle:"italic"}}>{post.caption}</p>
                  </div>}
                  <button onClick={()=>deleteVisionPost(post.id)}
                    style={{position:"absolute",top:"6px",right:"6px",background:"rgba(26,22,18,0.7)",border:"none",color:"#f0e8d8",width:"24px",height:"24px",borderRadius:"50%",fontSize:"12px",cursor:"pointer"}}>×</button>
                </div>
              ))}
            </div>
          )}
      ...
      {celebrating&&<div className="fi" style={{position:"fixed",inset:0,background:"rgba(26,22,18,0.97)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:100}}>
